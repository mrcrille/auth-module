...
export default class Auth {
  constructor (ctx, options) {
    ...
    //   this.options.redirect[key] = '/' + ctx.app.i18n.locale + this.options.redirect[key]
    // }

    // ctx.app.i18n.onLanguageSwitched = (old, locale) => {
    //   console.log(this.ctx.app.router.app.localePath)
    //   console.log(this.ctx.app.router.app.localePath('home', locale))

    //   for (var key in this.options.redirect) {
    //     this.options.redirect[key] = this.options.redirect[key].replace('/' + old, '/' + locale)
    //   }
    // }
  ...
  redirect (name, noRouter = false) {
    ...
    if (this.options.i18n) {
      var path = this.options.redirect[name]
      ...
      to = this.ctx.app.router.app.localePath(path)
    }
